...
density <- read_csv("data/mafa_density_circumf.csv",
  ...
  rename(treatment = removal, 
         MAFA_density_2x2 = MAFA_density) %>% 
  dplyr::filter(treatment == "no") %>% # density = 2x2 = 4m^2
  mutate(
    MAFA_density_1m2 = MAFA_density_2x2/4,
    MAFA_density_100m2 = (MAFA_density_2x2/4)*100
    )
...
hist(density$MAFA_density_100m2)
ggqqplot(density$MAFA_density_100m2)
shapiro.test(density$MAFA_density_100m2)
# Shapiro-Wilk normality test
# 
...
wilcox.test(MAFA_density_100m2 ~ site, data=density) 
...
density_table <- density %>%  
  dplyr::group_by(site) %>% 
  dplyr::summarize(avg_mafadensity = mean(MAFA_density_100m2), 
            SD = sd(MAFA_density_100m2),
            N = length(MAFA_density_100m2), 
            SE = ((sd(MAFA_density_100m2))/sqrt(length((MAFA_density_100m2)))))
...
density_table %>%
 ...
 kbl(caption = "MAFA density by site (#/100 m^2", digits = 1) %>%
 kable_classic(full_width = F, html_font = "Cambria")
