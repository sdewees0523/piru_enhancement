...
par_line <- ggplot(data = par_meanSD, aes(x = month, y = mean, group=treatment)) +
  ...
  theme_classic() +
  labs(title="Normalized Photosynthetically Active Radiation (electrons/m2/s)",x="Month", y = "Normalized PAR")
...
par_cover_scatter <- ggplot(data = par.2019, aes(x = cover_percent, y = normalized_par, color = month.x)) +
  geom_point(size=2, shape=16) +
 # scale_color_manual(values=c('#D3E3CA', '#92A587', '#2F3525', '#9F7E75', '#B9B994', '#F19B34')) +
  theme(legend.position="top") +
  theme_classic() +
  labs(title="Normalized Photosynthetically Active Radiation (electrons/m2/s)",x="MFAF Cover (%)", y = "Normalized PAR")
