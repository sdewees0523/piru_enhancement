...
leaftraits_nmdsprep <- leaftraits %>% # data needed for NMDS. columns: species, plant_rep, sla, dlmc, leaf thickness
  select(!c(individual_ID, leaf_number, leaf_rep, area_m2, leaf_LDMC_proportion))

leaftraits_nmdsprep_num <- leaftraits %>% # numeric only data needed for NMDS
  # use the ungroup() function to take out grouping
  #ungroup() %>% 
  select(!c(species, individual_ID, leaf_number, plant_rep, leaf_rep, area_m2, leaf_LDMC_proportion))

view(leaftraits_nmdsprep_num) # only numbers
str(leaftraits_nmdsprep_num) #tibble [64 × 3]
...
nmds_leaftraits_df_plotcolor <- ggplot(nmds_leaftraits_df, aes(x = MDS1, y = MDS2, color = species)) + 
...
# new plot
nmds_leaftraits_df_plotcolor_new <- ggplot(nmds_leaftraits_df, aes(x = MDS1, y = MDS2, color = species)) + 
  coord_equal() +
  geom_point(size = 3, alpha = 0.8, aes(shape = plant_rep)) +
  scale_color_manual(values = c("darkgreen", "darkmagenta", "darkgoldenrod1", "darkblue")) + # in color
 #scale_color_manual(values = c("gray0", "gray30", "gray60", "gray80")) + # in bw
 # theme(legend.position = c(0.08, 0.81)) + # x, y bottom left corner: 0,0 - IF want to set key within plot, otherwise sits outside figure
  stat_ellipse(level = .95, linetype = 1, linewidth = 1) + # linetype: 1 = solid, 2 = dashed
  geom_segment(data = fit_leaftraits_pvals, aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.25, "cm")),
               col = "black") +
   geom_text(data = fit_leaftraits_pvals, aes(label = species),  
...
```{r fortify nmds}
#basic normal plot
ordiplot(leaftraits_NMDS) # without point labels
...
#this makes a dataframe that gives a lot of control graphically 
leaftraits_NMDS_fort <- fortify(nmds_leaftraits_df)
