...
leaftraits_nmdsprep_num <- leaftraits %>% 
  # use the ungroup() function to take out grouping
  #ungroup() %>% 
  select(!c(species, individual_ID, leaf_number, area_m2, leaf_LDMC_proportion))
...
nmds_leaftraits_df_plotcolor <- ggplot(nmds_leaftraits_df, aes(x = MDS1, y = MDS2, color = species, shape = individual_ID)) + #, shape = individual_ID
