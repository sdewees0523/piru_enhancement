...
nncover_may2018 <- dplyr::filter(exotic_cover_all, year == "2018") %>% # nncover may 2018
  select(site, plot_number, treatment, cover_percent)
...
growth_march_may <- right_join(height_march, height_may, by = c("plot_number", "MAFA_removal", "location", "species", "individual")) %>% 
  ...
  select(plot_number, MAFA_removal, location, species, individual, rgr, normalized_par) %>% 
...
rhov_model <- glmer(as.factor(may_alive) ~ march_stem_height_cm + soil_moisture + (1|plot_number),
          family = binomial,
          data = rhov_2018)
summary(rhov_model)
