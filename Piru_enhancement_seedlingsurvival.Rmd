...
# soil moisture
summary(aov(may_alive ~ soil_moisture, data = height_march_surv_may_notransf))
#               Df Sum Sq Mean Sq F value   Pr(>F)    
...
TukeyHSD(aov(soil_moisture ~ may_alive, data = height_march_surv_may_notransf))
...
mayscatter.1 <- ggplot(data = height_march_surv_may, aes(x = soil_moisture, y = normalized_par)) +
  geom_point(aes(shape = may_alive)) +
