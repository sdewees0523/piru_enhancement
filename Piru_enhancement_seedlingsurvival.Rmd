...
```{r subsetting data by year and month, include=TRUE}
...
tdr_april2018 <- dplyr::filter(tdr, year == "2018" & month =="4")
...
survmortnospp_may2018_partdr <- survmortnospp_may2018_par %>% 
  dplyr::left_join(tdr_april2018, by = c("year", "site", "plot_number", "treatment"), na.rm = F) 


%>% 
  rename(month_survmort = month.x, 
         month_tdr = month.y)

print(survmortnospp_may2018_mod)
