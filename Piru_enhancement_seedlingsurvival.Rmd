...
glme.may2018.mod <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm  + (1|plot_number), 
                         family = binomial,
...
summary(glmer(as.factor(may_alive) ~ normalized_par +  (1|plot_number), 
                         family = binomial,
                         data = ceol_2018))

summary(glmer(as.factor(may_alive) ~ soil_moisture + (1|plot_number),
          family = binomial,
          data = ceol_2018))

summary(glmer(as.factor(may_alive) ~ exotic_percentcover + (1|plot_number),
          family = binomial, 
          data = ceol_2018))
...
summary(glmer(as.factor(may_alive) ~ march_stem_height_cm + (1|plot_number),
          family = binomial,
          data = ceol_2018))
...
AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + normalized_par + (1|plot_number),
          family = binomial,
          data = rhov_2018))
...
AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + exotic_percentcover + (1|plot_number),
          family = binomial,
          data = rhov_2018))

AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + site + (1|plot_number),
          family = binomial,
          data = rhov_2018))

AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + exotic_percentcover + normalized_par + (1|plot_number),
          family = binomial,
          data = rhov_2018))

AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + exotic_percentcover + soil_moisture + (1|plot_number),
          family = binomial,
          data = rhov_2018))

AIC(glmer(as.factor(may_alive) ~ march_stem_height_cm + exotic_percentcover + site + (1|plot_number),
          family = binomial,
          data = rhov_2018))
...
summary(glmer(as.factor(may_alive) ~ march_stem_height_cm + exotic_percentcover + (1|plot_number),
          family = binomial,
          data = rhov_2018))

rhov_glme <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm + (1|plot_number), 
                         family = binomial,
                         data = rhov_2018)
...
hear_glme <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm + (1|plot_number), 
                         family = binomial,
                         data = hear_2018)
...
saap_glme <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm + (1|plot_number),
                         family = binomial,
                         data = saap_2018)

summary(saap_glme)

hear_saap <- height_march_surv_may %>% 
  filter(species %in% c("SAAP", "HEAR"))

glme <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm + (1|plot_number),
                         family = binomial,
                         data = hear_saap)
summary(glme)
```
...
height_march_surv_may_2019 <- full_join(height_march_2019, survival_may_2019, by = c("plot_number", "MAFA_removal", "location", "species")) %>% 
  ...
  mutate(plot_number = as.character(plot_number)) %>% 
  left_join(par_april2018, by = "plot_number") %>% 
  select(plot_number, MAFA_removal, location, species, march_stem_height_cm, may_alive, normalized_par) %>% 
  left_join(tdr_april2018, by = "plot_number") %>% 
  select(!year & !site & !month & !treatment) %>% 
  left_join(nncover_may2018, by = "plot_number") %>% 
  select(!year & !date & !treatment) %>% 
...
```{r}
glme.may2019.mod <- glmer(as.factor(may_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + march_stem_height_cm + (1|plot_number), 
                         family = binomial,
...
site_1_sm_plot <- ggplot()+
  geom_point(data = height_march_surv_may_2019 %>%  filter(site == "1"), aes(x = soil_moisture, y = may_alive), alpha = 0.25)+
  geom_line(data = soil_moisture_site_1_prediction, aes(x = soil_moisture, y = alive), linewidth = 1)+
  theme_classic()+
  labs(x = "Soil Moisture (%)",
       y = "Probability of Survival",
...
site_2_sm_plot <- ggplot()+
  geom_point(data = height_march_surv_may_2019 %>%  filter(site == "2"), aes(x = soil_moisture, y = may_alive), alpha = 0.25)+
  geom_line(data = soil_moisture_site_2_prediction, aes(x = soil_moisture, y = alive), linewidth = 1)+
  theme_classic()+
  labs(x = "Soil Moisture (%)",
       y = "Probability of Survival",
...
site_1_sm_plot + site_2_sm_plot
## stem height
stem_height_site_1_prediction <- seq(from = 0.05, to = 11, by = 0.1) %>% 
  as.data.frame() %>% 
  rename(march_stem_height_cm = ".") %>% 
  mutate(normalized_par = mean(height_march_surv_may_2019$normalized_par),
         exotic_percentcover = mean(height_march_surv_may_2019$exotic_percentcover),
...
stem_height_site_2_prediction <- seq(from = 0.05, to = 11, by = 0.1) %>% 
  as.data.frame() %>% 
  rename(march_stem_height_cm = ".") %>% 
  mutate(normalized_par = mean(height_march_surv_may_2019$normalized_par),
         exotic_percentcover = mean(height_march_surv_may_2019$exotic_percentcover),
...
height_may_surv_sep_2019 <- full_join(height_may_2019, survival_sep_2019, by = c("plot_number", "MAFA_removal", "location", "species")) %>% 
  ...
  mutate(plot_number = as.character(plot_number)) %>% 
  left_join(par_april2018, by = "plot_number") %>% 
  select(plot_number, MAFA_removal, location, species, may_stem_height_cm, sep_alive, normalized_par) %>% 
  left_join(tdr_april2018, by = "plot_number") %>% 
  select(!year & !site & !month & !treatment) %>% 
  left_join(nncover_may2018, by = "plot_number") %>% 
  select(!year & !date & !treatment) %>% 
...
```{r}
glme.sep2019.mod <- glmer(as.factor(sep_alive) ~ normalized_par + soil_moisture + exotic_percentcover + site + may_stem_height_cm + (1|plot_number), 
                         family = binomial,
