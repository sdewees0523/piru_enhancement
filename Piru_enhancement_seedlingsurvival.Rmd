...
#  select(plot_number, year, cover_percent, site) %>% 
  ...
  dplyr::select(plot_number, year, mafacover, site)
print(mafa_cover_all)
```
### A2. site differences - overall
```{r mafa_cover_all differences - normality, include=TRUE}
# mafacover by site
hist(mafa_cover_all$mafacover)
ggqqplot(mafa_cover_all$mafacover)
shapiro.test(mafa_cover_all$mafacover)
# Shapiro-Wilk normality test
# 
# data:  mafa_cover_all$mafacover
# W = 0.929, p-value = 0.0005531 <-- not normally distributed
```

```{r mafa_cover_all differences - stats, include=TRUE}
wilcox.test(mafacover ~ site, data=mafa_cover_all) 
# Wilcoxon rank sum test with continuity correction
# 
# data:  mafacover by site
# W = 1036.5, p-value = 1.217e-05
# alternative hypothesis: true location shift is not equal to 0

mafa_cover_all_table <- mafa_cover_all %>%  
  dplyr::group_by(site) %>% 
...
# 1	    31.87500	    17.33344	36	2.888907
# 2	    14.13889	    15.72026	36	2.620043

### print in Console
mafa_cover_all_table %>%
...
summary(glme.may2018.modspp_CEOL) # not this one. go to Chunk D3. survival by species
...
summary(aov(rgr ~ treatment, data = growth_march_may))
#              Df   Sum Sq   Mean Sq F value Pr(>F)  
# treatment     2 0.000475 2.377e-04    3.05 0.0488 *
# Residuals   306 0.023848 7.794e-05  
TukeyHSD(aov(rgr ~ treatment, data = growth_march_may))
# $treatment
#                                    diff          lwr          upr     p adj
# half removal-full removal -0.0003549849 -0.003118235 0.0024082651 0.9507993
# none-full removal         -0.0029568062 -0.005940885 0.0000272722 0.0527460
# none-half removal         -0.0026018213 -0.005647265 0.0004436226 0.1109977

growth_march_may %>%  
  group_by(treatment) %>% 
  summarize(avg_vol = mean(), 
            avg_rgr = mean(rgr), 
            N = length(rgr), 
            se = ((sd(rgr))/sqrt(length((rgr)))), 
            min = min(rgr), 
            max = max(rgr))
...
#              Df  Sum Sq Mean Sq F value Pr(>F)    
# species       3 1172091  390697   101.7 <2e-16 ***
# Residuals   390 1498285    3842  
#$species
#                 diff        lwr        upr     p adj
...
print(height_jan_surv_may_2019_notransf %>%  
  ...
  summarise(N = length(may2019_alive), 
            ...
            total_died = sum(may2019_alive == "0"),
            percent_survived = 100*(sum(may2019_alive == "1")/length(may2019_alive))))
# treatment. N. total_survived. total_died. percent_survived
# full	130	82	48	63.07692
# half	132	69	63	52.27273
# none	132	78	54	59.09091

summary(aov(may2019_alive ~ treatment, data = height_jan_surv_may_2019_notransf))
# Df Sum Sq Mean Sq F value               Pr(>F)
# treatment     2   0.78  0.3916    1.61  0.201
# Residuals   391  95.12  0.2433
...
summary(aov(rgr ~ MAFA_removal, data = growth_jan_may2019))
#  Df   Sum Sq   Mean Sq F value Pr(>F)
# MAFA_removal   2 0.000013 6.378e-06   0.271  0.763
# Residuals    234 0.005508 2.354e-05   
TukeyHSD(aov(rgr ~ MAFA_removal, data = growth_jan_may2019))
...
# rgr of SAAP by species in Jan - May 2019
summary(aov(rgr ~ MAFA_removal, data = growth_jan_may2019 %>%  filter (species == "SAAP")))
#  Df    Sum Sq   Mean Sq F value Pr(>F)
# MAFA_removal  2 0.0000048 2.409e-06    0.09  0.914
# Residuals    68 0.0018232 2.681e-05
...
# $MAFA_removal
#                                    diff          lwr         upr     p adj
# half removal-full removal  2.669098e-05 -0.003663431 0.003716813 0.9998344
# none-full removal         -5.333383e-04 -0.004008635 0.002941958 0.9282587
# none-half removal         -5.600293e-04 -0.004282101 0.003162042 0.9309377
```
...
#Residuals   87 0.0006133 7.05e-06      
TukeyHSD(aov(rgr ~ species, data = growth_may_nov2019))
#        Fit: aov(formula = rgr ~ species, data = growth_may_nov2019)
# 
# $species
#                    diff          lwr          upr     p adj
# HEAR-CEOL -0.0019100313 -0.004470106 0.0006500430 0.2134559
...
summary(aov(rgr ~ MAFA_removal, data = growth_may_nov2019))   
#              Df    Sum Sq   Mean Sq F value Pr(>F)  
# MAFA_removal  2 0.0000512 2.558e-05   3.762 0.0271 *
# Residuals    88 0.0005984 6.800e-06    
TukeyHSD(aov(rgr ~ MAFA_removal, data = growth_may_nov2019))    
#                                    diff          lwr           upr     p adj
# half removal-full removal -0.0009948853 -0.002594934  0.0006051633 0.3042888
# none-full removal         -0.0019316405 -0.003610719 -0.0002525623 0.0200030
# none-half removal         -0.0009367552 -0.002488005  0.0006144944 0.3251929

growth_may_nov2019 %>%  
  group_by(MAFA_removal) %>% 
  summarize(avg_rgr = mean(rgr), 
            SD = sd(rgr),
            N = length(rgr), 
            SE = ((sd(rgr))/sqrt(length((rgr)))))
# MAFA_removal.   avg_rgr.      SD.       N.    SE
# full removal	   0.0007383069	0.002997334	26	0.0005878255
# half removal	  -0.0002565784	0.002316716	36	0.0003861193
# none        	  -0.0011933336	0.002577264	29	0.0004785858
...
# Df Sum Sq Mean Sq F value Pr(>F)    
# species       3  809.8  269.95    48.1 <2e-16 ***
# Residuals   232 1302.1    5.61   
...
# Df  Sum Sq Mean Sq F value   Pr(>F)    
# species       3  569523  189841   18.04 1.48e-10 ***
# Residuals   232 2442042   10526 
...
# SURVIVAL
view(height_may_surv_nov_2019_notransf %>%  
  group_by(species, treatment) %>% 
...
# species.  N.  total_survived.  total_died.  percent_survived
# CEOL	40	6	34	15.00000
# HEAR	81	33	48	40.74074
# RHOV	45	6	39	13.33333
# SAAP	70	32	38	45.71429
view(height_may_surv_nov_2019_notransf %>%  
  group_by(species, treatment) %>% 
  summarise(N = length(nov_alive), 
            total_survived = sum(nov_alive == "1"), 
...
# Df Sum Sq Mean Sq F value   Pr(>F)    
# species       3   4.65  1.5500   7.615 7.06e-05 ***
# Residuals   232  47.23  0.2036  
...
print(height_may_surv_nov_2019_notransf %>%  
  ...
  summarise(N = length(nov_alive), 
            ...
            total_died = sum(nov_alive == "0"),
            percent_survived = 100*(sum(nov_alive == "1")/length(nov_alive))))
# site       N.   mean_survival   total_survived.  total_died.  percent_survived
# 1	        148	  0.3986486	      59	              89	        39.86486
# 2	        88	  0.2045455	      18	              70	        20.45455
...
print(height_may_surv_nov_2019_notransf %>%  
  ...
  summarise(N = length(nov_alive), 
            ...
            percent_survived = 100*(sum(nov_alive == "1")/length(nov_alive))))
# species.  N.  total_survived.  total_died.  percent_survived
# full	84	0.2619048	22	62	26.19048
# half	72	0.4583333	33	39	45.83333
# none	80	0.2750000	22	58	27.50000
summary(aov(nov_alive ~ treatment, data = height_may_surv_nov_2019_notransf))
#              Df Sum Sq Mean Sq F value Pr(>F)  
# treatment     2   1.81  0.9070   4.221 0.0158 *
# Residuals   233  50.06  0.2149  
...
glme.nov2019.mod1_SAAP_2 <- glmer(as.factor(nov_alive) ~ normalized_par  + site + may_volume_cm3  
                             + (1|plot_number), 
                         family = binomial(link = "logit"), 
                         data = (height_may_surv_nov_2019 %>% filter(species == "SAAP")), na.action = "na.fail")
# boundary (singular) fit: see help('isSingular')

glme.nov2019.mod1_SAAP_3 <- glmer(as.factor(nov_alive) ~ normalized_par  + site   
                             + (1|plot_number), 
                         family = binomial(link = "logit"), 
                         data = (height_may_surv_nov_2019 %>% filter(species == "SAAP")), na.action = "na.fail")
# boundary (singular) fit: see help('isSingular')

glme.nov2019.mod1_SAAP_4 <- glmer(as.factor(nov_alive) ~ may_volume_cm3  + site   
                             + (1|plot_number), 
                         family = binomial(link = "logit"), 
                         data = (height_may_surv_nov_2019 %>% filter(species == "SAAP")), na.action = "na.fail")
# boundary (singular) fit: see help('isSingular')
