...
```{r}
height <- read.csv(here("data", "seedlingheight_survival_data.csv")) %>% 
...
lme.may2018.mod <- lmer(totalsurvival ~ normalized_par + soil_moisture + exotic_percentcover + site + (1|treatment), 
                        data = survmortnospp_may2018_join) # <-- DO I NEED TO INCLUDE TREATMENT IF IT WASN'T SIG? 
...
emmeans(lme.may2018.mod, list(pairwise ~ site), adjust = "tukey")
# site is not significantly different (p = 0.0923)

# calculate mean survival at Site 1 and Site 2
survmortnospp_may2018_join %>% 
  group_by(site) %>% 
  summarise(MEAN_totalsurvival=mean(totalsurvival, na.rm = TRUE),
            SD = sd(totalsurvival, na.rm = TRUE), 
            N = length(totalsurvival), 
            SE = ((sd(totalsurvival, na.rm = TRUE))/sqrt(length((totalsurvival))))
  )
