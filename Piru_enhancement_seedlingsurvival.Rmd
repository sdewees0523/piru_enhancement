...
survival_may <- height_raw %>%  ## Need to change some of the individual labels to fix NAs when rejoin. 
  ...
  dplyr::select(plot_number, MAFA_removal, location, species, alive, individual, volume_cm3) %>% 
  rename(may_alive = alive, 
         may_volume_cm3 = volume_cm3) %>% # 1 = alive
...
height_march_surv_may_notransf <- full_join(height_march, survival_may, by = c("plot_number", "MAFA_removal", "location", "species", "individual")) %>% 
  ...
  dplyr::select(plot_number, MAFA_removal, location, species, individual, march_stem_height_cm, may_alive, normalized_par, march_volume_cm3, may_volume_cm3) %>% 
...
nncover_may2019 <- dplyr::filter(exotic_cover_all, year == "2019") %>% 
  dplyr::select(site, plot_number, MAFA_removal, cover_percent) %>% 
  rename(treatment = MAFA_removal)

# join site conditions 2019
site_cond_2019 <- full_join(par_may2019, tdr_march2019, by = c("site", "plot_number", "treatment")) %>% 
  full_join(nncover_may2018, by = c("site", "plot_number", "treatment"))
glimpse(site_cond_2019)

write.csv(site_cond_2019, "data/processed/may2019_site_conditions.csv")
```
...
survival_may_2019 <- height_raw %>% 
  ...
  dplyr::select(plot_number, MAFA_removal, location, species, alive, volume_cm3) %>% 
  rename(may_alive = alive, may_volume_cm3 = volume_cm3) %>% 
...
growth_jan_may2019 <- right_join(height_jan_2019, height_may_2019, by = c("plot_number", "MAFA_removal", "location", "species", "individual")) %>% 
  mutate(jan_stem_height_cm = as.numeric(jan_stem_height_cm)) %>% 
  drop_na() %>% 
  mutate(rgr = (log(may_stem_height_cm) - log(jan_stem_height_cm))/, # relative growth rate per day equation; 60 days between March and May
         plot_number = as.character(plot_number)) %>% 
  left_join(par_april2018, by = "plot_number") %>% 
  dplyr::select(plot_number, MAFA_removal, location, species, individual, rgr, normalized_par) %>% 
  left_join(tdr_april2018, by = "plot_number") %>% 
  ...
  left_join(nncover_may2018, by = "plot_number") %>% 
  dplyr::select(!treatment) %>% 
  rename(exotic_percentcover = cov```er_percent) %>% 
  rename(treatment = MAFA_removal)
```
...
height_jan_surv_may_2019 <- full_join(height_jan_2019, survival_may_2019, by = c("plot_number", "MAFA_removal", "location", "species")) %>% 
  ...
  mutate(plot_number = as.character(plot_number))%>% 
  left_join(par_may2019, by = "plot_number") %>% 
  dplyr::select(plot_number, MAFA_removal, location, species, jan_stem_height_cm, may_alive, normalized_par, jan_volume_cm3, may_volume_cm3) %>% 
  left_join(tdr_march2019, by = "plot_number") %>% 
  dplyr::select(!site & !treatment) %>% 
  left_join(nncover_may2019, by = "plot_number") %>% 
  dplyr::select(!treatment) %>% 
  rename(exotic_percentcover = cover_percent) %>% 
  rename(treatment = MAFA_removal) %>% 
