...
```{r may 2018 filter data, include=TRUE}
...
#seedling height in March 2018 - START
...
#seedling height in May 2018 - END
...
# growth rate from March to May 2018
...
# stephanie start here - start with height_march and survival_may and figure out the code to the output of height_march_surv_may
height_march_surv_may <- full_join(height_march, survival_may, by = c("plot_number", "MAFA_removal", "location", "species", "individual")) %>% 
  ...
  rename(exotic_percentcover = cover_percent) %>% 
  rename(treatment = MAFA_removal) %>% 
  mutate(normalized_par = scale(normalized_par), # scale transforms the data; helps when comparing variables with different units
...
glme.may2018.mod <- glmer(as.factor(may_alive) ~ normalized_par * soil_moisture + exotic_percentcover + site + march_stem_height_cm + species + (1|plot_number), 
                         family = binomial,
