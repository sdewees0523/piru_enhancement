...
write.csv(site_cond, "data/processed/may2018_site_conditions.csv")
```
...
height_march <- height_raw %>% 
  ...
  rename(march_stem_height_cm = stem_height_cm, march_volume_cm3 = volume_cm3) %>% 
  drop_na()
...
height_march_surv_may_notransf <- full_join(height_march, survival_may, by = c("plot_number", "MAFA_removal", "location", "species", "individual")) %>% 
  ...
  dplyr::select(plot_number, MAFA_removal, location, species, individual, march_stem_height_cm, may_alive, normalized_par, march_volume_cm3) %>% 
