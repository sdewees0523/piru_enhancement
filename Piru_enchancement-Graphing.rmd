...
survival_species_2018 <- ggplot(data = surv_2018_proportion_long_meanse,
  ...
  # geom_point(aes(fill = species), size=4) + #color = species, 
  ...
  scale_fill_manual(values = c("transparent", "gray90", "gray40", "black"),
                    name = "Species", 
                     labels = c(expression(italic("Ceanothus oliganthus")), 
                               expression(italic("Heteromeles arbutifolia")), 
                               expression(italic("Rhus ovata")), 
                               expression(italic("Salvia apiana")))) +
  scale_color_manual(values = c("transparent", "gray90", "gray40", "black"),
                     name = "Species", 
                     labels = c(expression(italic("Ceanothus oliganthus")), 
                               expression(italic("Heteromeles arbutifolia")), 
                               expression(italic("Rhus ovata")), 
...
#ggsave("figures/aliveseedlings2018.png", survival_species_2018, width = 8, height = 4)

print(class(survival_species_2018)) # <-- need to change to gg, ggplot to group in a combined figure with survival_2019
...
survival_species_2019 <- 
  ...
  aes(x = month_proportion, 
      ...
      shape = species, 
      fill = species)) +
    geom_line(size = 1) + # color = species
   geom_point(size=4) +
...
#ggsave("figures/aliveseedlings2019.png", survival_species_2019, width = 8, height = 4)
```

```{r}
figure <- ggarrange(survival_species_2018, survival_species_2018, survival_species_2018,
                    labels = c("A", "B", "C"),
                    ncol = 2, nrow = 2)
figure
```
...
tdr_cover_scatter_2019 <- ggplot(
  data = surv_all.2019,
  aes(x = native_cover, y = soil_moisture, color = treatment)) +
  geom_point(size=2, shape=16) +
 # scale_color_manual(values=c('#D3E3CA', '#92A587', '#2F3525', '#9F7E75', '#B9B994', '#F19B34')) +
  theme(legend.position="top") +
  theme_classic() +
  labs(title="Soil Moisture by Treatment in March 2019",
       x="Canopy Cover (%)", y = "Soil Moisture (%)") +
  geom_smooth()
...
# survival x treatment
### 2018
```{r}
# Example data
initial_2018 <- initial %>% 
  filter(year == 2018)

surv_2018 <- surv_all %>% 
  filter(year == 2018) %>% 
  drop_na(species) %>%
  group_by(plot_number, treatment, species) %>%
  arrange(month_count) %>%
  mutate(
    deaths_this_month = dead - lag(dead, default = 0),
    deaths_this_month = if_else(deaths_this_month < 0, 0, deaths_this_month) # safety check
  )

# Step 2. Identify how many individuals were still alive at last obs
final_alive_2018 <- surv_2018 %>%
  filter(month_count == max(month_count)) %>%
  pull(alive)

# Step 3. Expand deaths into individual records
deaths_2018 <- surv_2018 %>%
  filter(deaths_this_month > 0) %>%
  select(plot_number, treatment, species, month_count, deaths_this_month) %>%
  uncount(deaths_this_month, .id = "individual_id") %>%
  mutate(event = 1)

# Step 4. Expand survivors (still alive at end)
alive_final_2018 <- surv_2018 %>%
  filter(month_count == max(month_count)) %>%
  select(plot_number, treatment, species, month_count, alive) %>%
  uncount(alive, .id = "individual_id") %>%
  mutate(event = 0)

# Step 5. Combine to full survival dataframe
survival_df_2018 <- bind_rows(deaths_2018, alive_final_2018) %>%
  rename(time = month_count)

hear_survival_2018 <- survival_df_2018 %>% 
  filter(species == "HEAR")

# Step 6. Fit survival model by treatment
hear_surv_obj_2018 <- Surv(time = hear_survival_2018$time, event = hear_survival_2018$event)
hear_fit_2018 <- survfit(hear_surv_obj_2018 ~ treatment, data = hear_survival_2018)

# Optional plot
ggsurvplot(hear_fit_2018, data = hear_survival_2018,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

saap_survival_2018 <- survival_df_2018 %>% 
  filter(species == "SAAP")

# Step 6. Fit survival model by treatment
saap_surv_obj_2018 <- Surv(time = saap_survival_2018$time, event = saap_survival_2018$event)
saap_fit_2018 <- survfit(saap_surv_obj_2018 ~ treatment, data = saap_survival_2018)

# Optional plot
ggsurvplot(saap_fit_2018, data = saap_survival_2018,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)
...
# Step 6. Fit survival model by treatment
total_surv_obj_2018 <- Surv(time = survival_df_2018$time, event = survival_df_2018$event)

total_fit_2018 <- survfit(total_surv_obj_2018 ~ treatment, data = survival_df_2018)

# Optional plot
ggsurvplot(total_fit_2018, data = survival_df_2018,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)
...
surv_2019 <- surv_all %>% 
  ...
  drop_na(species) %>%
  group_by(plot_number, treatment, species) %>%
  mutate(month_count = month_count - 12) %>% 
  arrange(month_count) %>%
  mutate(
    deaths_this_month = dead - lag(dead, default = 0),
    deaths_this_month = if_else(deaths_this_month < 0, 0, deaths_this_month) # safety check
...
# Step 2. Identify how many individuals were still alive at last obs
final_alive_2019 <- surv_2019 %>%
  filter(month_count == max(month_count)) %>%
  pull(alive)

# Step 3. Expand deaths into individual records
deaths_2019 <- surv_2019 %>%
  filter(deaths_this_month > 0) %>%
  select(plot_number, treatment, native_cover, species, month_count, deaths_this_month) %>%
  uncount(deaths_this_month, .id = "individual_id") %>%
  mutate(event = 1)

# Step 4. Expand survivors (still alive at end)
alive_final_2019 <- surv_2019 %>%
  filter(month_count == max(month_count)) %>%
  select(plot_number, treatment, native_cover, species, month_count, alive) %>%
  uncount(alive, .id = "individual_id") %>%
  mutate(event = 0)

# Step 5. Combine to full survival dataframe
survival_df_2019 <- bind_rows(deaths_2019, alive_final_2019) %>%
  rename(time = month_count)

hear_survival_2019 <- survival_df_2019 %>% 
  filter(species == "HEAR") 

hear_cover_2019 <- hear_survival_2019 %>% 
  filter(native_cover %in% c(min(native_cover), median(native_cover), max(native_cover))) %>% 
  mutate(native_cover = as.character(native_cover))

# Step 6. Fit survival model by treatment
hear_surv_obj_2019 <- Surv(time = hear_survival_2019$time, event = hear_survival_2019$event)
hear_treatment_2019 <- survfit(hear_surv_obj_2019 ~ treatment, data = hear_survival_2019)
hear_surv_obj_2019_2 <- Surv(time = hear_cover_2019$time, event = hear_cover_2019$event)
hear_native_cover_2019 <- survfit(hear_surv_obj_2019_2 ~ native_cover, data = hear_cover_2019)
# Optional plot
ggsurvplot(hear_treatment_2019, data = hear_survival_2019,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

ggsurvplot(hear_native_cover_2019, data = hear_cover_2019,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

saap_survival_2019 <- survival_df_2019 %>% 
  filter(species == "SAAP") 

saap_cover_2019 <- saap_survival_2019 %>% 
  filter(native_cover %in% c(min(native_cover), median(native_cover), max(native_cover))) %>% 
  mutate(native_cover = as.character(native_cover))

# Step 6. Fit survival model by treatment
saap_surv_obj_2019 <- Surv(time = saap_survival_2019$time, event = saap_survival_2019$event)
saap_treatment_2019 <- survfit(saap_surv_obj_2019 ~ treatment, data = saap_survival_2019)

saap_surv_obj_2019_2 <- Surv(time = saap_cover_2019$time, event = saap_cover_2019$event)
saap_native_cover_2019 <- survfit(saap_surv_obj_2019_2 ~ native_cover, data = saap_cover_2019)

# Optional plot
ggsurvplot(saap_treatment_2019, data = saap_survival_2019,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

ggsurvplot(saap_native_cover_2019, data = saap_cover_2019,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)
...
### full year
# Step 6. Fit survival model by treatment
total_surv_obj_2019 <- Surv(time = survival_df_2019$time, event = survival_df_2019$event)
total_treatment_2019 <- survfit(total_surv_obj_2019 ~ treatment, data = survival_df_2019)
total_surv_obj_2019_2 <- Surv(time = total_cover_2019$time, event = total_cover_2019$event)
total_native_cover_2019 <- survfit(total_surv_obj_2019_2 ~ native_cover, data = total_cover_2019)

# Optional plot
ggsurvplot(total_treatment_2019, data = survival_df_2019,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)
...
### fall of 2019
surv_2019_fall <- survival_df_2019 %>% 
  filter(time > 3)
# Step 6. Fit survival model by treatment
total_surv_obj_2019_fall <- Surv(time = surv_2019_fall$time, event = surv_2019_fall$event)
total_treatment_2019_fall <- survfit(total_surv_obj_2019_fall ~ treatment, data = surv_2019_fall)

total_surv_obj_2019_2_fall <- Surv(time = surv_2019_fall$time, event = surv_2019_fall$event)
total_native_cover_2019 <- survfit(total_surv_obj_2019_2_fall ~ native_cover, data = surv_2019_fall)

# Optional plot
ggsurvplot(total_treatment_2019_fall, data = surv_2019_fall,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

ggsurvplot(total_native_cover_2019, data = surv_2019_fall,
           conf.int = TRUE,
           risk.table = F,
           pval = TRUE)

# ggsave("figures/total_surv_2019_treat_figure.pdf",
#        total_surv_2019_treat_figure$plot,
#        width = 8, height = 6)
...
```{r}
print("You're reached the end of the code!")
