...
surv_2019_treat <- surv_all %>%
  filter(year == "2019") %>%
  group_by(treatment) %>%
  summarise(
    mean_alive = mean(alive, na.rm = TRUE),
    sd_alive   = sd(alive, na.rm = TRUE),
    n_alive    = n()  # optional: sample size
  )

surv_2019_plot <- surv_all %>%
  group_by(site, treatment, plot_number) %>%
  summarise(total_alive = sum(alive, na.rm = TRUE))
 
# stats 2019
alive_by_plot <- surv_all %>%
  group_by(plot_number) %>%
  summarise(total_alive = sum(alive, na.rm = TRUE))

hist(surv_2019_plot$total_alive)
shapiro.test(surv_all$alive) # <- not normal
# not normal
kruskal.test(alive ~ treatment, data = surv_all) # <- p-value = 0.1427
# Pairwise comparisons (nonparametric post-hoc)
# pairwise.wilcox.test(surv_all$alive, surv_all$treatment, p.adjust.method = "BH") <-- doesn't run (Stephanie 11/5/2025)

# Plot mean ± SD survival per treatment
# ggplot(surv_2019_treat, 
#        aes(x = treatment, y = mean_alive)) +
#   geom_col(fill = "darkseagreen3", color = "black", width = 0.6) +
#   geom_errorbar(aes(ymin = mean_alive - sd_alive, ymax = mean_alive + sd_alive),
#                 width = 0.2, size = 0.8) +
#   labs(
#     x = "Treatment",
#     y = "Mean number of seedlings alive",
#     title = "Seedling survival by treatment (2019)"
#   ) +
