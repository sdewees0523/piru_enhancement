...
```{r par means not normalized, include=TRUE}
par_means <- par_load %>% 
  dplyr::select(date, plot, weeding, readingno, ambient_PAR_1, below_PAR_1) %>% 
  rename(plot_number = plot) %>% 
  rename(treatment = weeding) %>% 
...
par_minmax <- par_means %>%  
  summarize(min = min(below_par), 
            mean = mean(below_par),
            max = max(below_par))
...
# not very pretty table
#par_minmax_table <- kable(par_minmax, format = "html", escape = FALSE, digits = 0) %>%
#  kable_styling(bootstrap_options = c("striped", "hover"))
#print(par_minmax_table)
  
par_monthmeans <- par_means %>%  
  group_by(month) %>% 
  summarize(mean_par = mean(below_par), 
            SD = sd(below_par),
            N = length(below_par), 
            SE = ((sd(below_par))/sqrt(length((below_par)))))

par_monthmeans %>%
  kbl(caption = "PAR values by month", digits = 0) %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
surv_all %>% 
  filter(month_count == "39") %>% 
  summarize(totalalive = sum(alive))
# 9 alive

view(surv_all %>% 
  filter(month_count == "39" & alive  != "0"))
