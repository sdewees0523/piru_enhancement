...
ceol_prep <- seedlingsurvival %>% 
 ...
 # select(!month) %>% 
  filter(species == "CEOL") %>% 
  select("seedlingID", "timepoint_status", "stem_height_cm") %>% 
...
ceol_prep_2 <- seedlingsurvival %>% 
  filter(species == "CEOL") %>% 
  select("timepoint", "alive", "seedlingID", "stem_height_cm") %>% 
  mutate(stem_height_cm = as.numeric(stem_height_cm))
...
seedling_prep <- seedlingsurvival %>% 
 ...
 # select(!month) %>% 
  #filter(species == "CEOL") %>% 
  select("timepoint", "alive", "species", "seedlingID", "stem_height_cm") %>% 
  mutate(stem_height_cm = as.numeric(stem_height_cm))
...
seedlingsurvival_2019_prep <- seedlingsurvival_2019 %>% 
  select("species", "seedlingID", "timepoint_status", "status", "timepoint", "stem_height_cm") %>% 
  mutate(stem_height_cm = as.numeric(stem_height_cm))
