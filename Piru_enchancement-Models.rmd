...
parmod_exotdr <- parmod %>%
  dplyr::select(!date) %>% 
  group_by(year, site, plot_number, treatment) %>% 
  full_join(nnmod_nodate, by = c("year", "site", "plot_number", "treatment")) %>% 
    ungroup()
#   full_join(tdrmod, by = c("year", "month", "site", "plot_number", "treatment", "mafacover"))
   
view(parmod_exotdr)
write.csv(parmod_exotdr, "data/processed/parmod_exotdr.csv")
```
...
parmod$treatment <- as.factor(parmod$treatment)
parmod$month <- factor(parmod$month, levels = c(1,3,4,5,7,9,11))
parmod$site <- as.factor(parmod$site)
...
# year doesn't run becuase only 2 numbers
...
#(Intercept)    3.030e-01  4.799e-02  6.792e+01   6.313 2.40e-08 ***
#treatmenthalf -3.299e-03  4.580e-02  3.465e+01  -0.072 0.942996    
#treatmentnone -9.332e-03  4.829e-02  3.874e+01  -0.193 0.847789    
#mafacover     -3.251e-03  9.261e-04  1.377e+02  -3.510 0.000606 *** 
#site2          3.360e-02  4.016e-02  4.019e+01   0.837 0.407691    
#month3         2.274e-01  3.079e-02  2.447e+02   7.384 2.40e-12 ***
#month4         8.682e-02  3.315e-02  2.688e+02   2.619 0.009318 ** 
#month5         1.981e-01  3.079e-02  2.447e+02   6.432 6.54e-10 ***
#month7         1.662e-01  3.079e-02  2.447e+02   5.398 1.59e-07 ***
#month9         2.151e-01  2.736e-02  2.549e+02   7.859 1.09e-13 ***
#month11        7.380e-02  3.079e-02  2.447e+02   2.397 0.017288 *  
...
parmod.interaction <- lm(normalized_par ~ mafacover*month, data = parmod)
anova(parmod.interaction) # p-values for the interaction
# Analysis of Variance Table
#Response: normalized_par
#                 Df Sum Sq Mean Sq F value    Pr(>F)    
#mafacover         1 1.3002 1.30023 53.2054 3.242e-12 ***
#month             6 1.7738 0.29563 12.0973 4.632e-12 ***
#mafacover:month   6 0.4450 0.07416  3.0346  0.006871 ** 
#Residuals       274 6.6960 0.02444     
```
...
parscatter.6 <- ggplot(data = parmod, aes(x = mafacover, y = normalized_par)) + 
  geom_point() +
