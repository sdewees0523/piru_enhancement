...
parmod_withnncover <- full_join(parmod, nnmod_nodate, by = c("year", "site", "plot_number", "treatment", "mafacover"))
view(parmod_withnncover)
parmod_withnncover %>%  # check number of data points
  group_by(year, treatment) %>% 
  summarise(N_mafacover = length(mafacover), 
            N_exocover = length(nncover))
write.csv(parmod_withnncover, "data/processed/parmod_exotdr.csv")
```
